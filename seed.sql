...
-- FIX: Reference caretakers table correctly.
CREATE TABLE matches (
  id SERIAL PRIMARY KEY,
  patient_id INT NOT NULL REFERENCES clients(id) ON DELETE CASCADE,
  caregiver_id INT NOT NULL REFERENCES caretakers(id) ON DELETE CASCADE, 
  match_score NUMERIC(5, 2) NOT NULL, -- Score from 0.00 to 100.00
  compatibility_factors JSONB, -- Stores the JSON output from the AI agent
  created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT unique_match_pair UNIQUE (patient_id, caregiver_id)
);
...
-- Maria Rodriguez's services (ID 3)
INSERT INTO caretaker_adls (caretaker_id, service_type, is_available, notes) VALUES
